...
a <- matrix(1:10, byrow=TRUE, ncol=2)
```
...
```{r}
# Put your code in here
b <- matrix(1:10, byrow=TRUE, ncol=5)
```
...
```{r}
# Put your code in here
d <- matrix(1:4,byrow=TRUE, ncol=2)
e <- matrix(5:8, byrow=TRUE, ncol=2)
f <- d*e
g <- d%*%e
#standard multiplication is just a scalar multiplication while matrix multiplication operator is the proper one
```
...
```{r}
# Put your code in here
q <- matrix(data=c(5,-2,3,-1,1,4,-1,1,4,0.5,12,-6,2,-1,1,-3), nrow=4, byrow=TRUE)
r <- c(2,19,-11,-12)
solve(q,r)
```
...
deriv(expression(x^2-log(x)-3*x),'x')
...
Get together with one of your classmates. Look at the dice you got. Now you need to come up with a game using them.
